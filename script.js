...
document.addEventListener("DOMContentLoaded", function () {
	...
	const DOM = {
		...
		containerTemplateSection: document.getElementById("containerTemplateSection"),
		containerTopTemplateInput: document.getElementById("containerTopTemplateInput"),
		containerBottomTemplateInput: document.getElementById("containerBottomTemplateInput"),
	...
	const {
		...
		containerTemplateSection, containerTopTemplateInput, containerBottomTemplateInput,
	...
	const validateTemplate = (template, tagType, allowScriptTags = false) => {
		...
		const requiredPlaceholders = {
			...
			'container': ['[CONTENT]'] // コンテナモードは[CONTENT]が必須
		};
	...
	const saveInputAreas = () => {
		...
		try {
			...
			allGroups.forEach((group) => {
				...
				// コンテナモードの場合は、選択された子エリアIDも保存
				let content = textarea.innerHTML;
				if (tagInfo && tagInfo.tagType === "container") {
					const childAreasInput = group.querySelector(".container-child-areas");
					if (childAreasInput) {
						content = childAreasInput.value; // 子エリアIDリストを保存
					}
				}

				// カスタム名前を取得
				const customName = group.getAttribute("data-custom-name") || "";

				// コンテナ内にある子エリアかどうかをチェック
				const isInsideContainer = group.closest(".container-children-zone") !== null;
	...
	const openTagModal = (button = null) => {
		...
		if (button) {
			...
			containerTopTemplateInput.value = button.containerTop || '';
			containerBottomTemplateInput.value = button.containerBottom || '';
			...
			containerTopTemplateInput.value = '';
			containerBottomTemplateInput.value = '';
	...
	const updateTemplatePlaceholder = () => {
		...
		// container用のフィールドの表示/非表示を切り替え
		if (selectedType === "container") {
			containerTemplateSection.style.display = "block";
			// コンテナモードではメインのテンプレート入力を非表示に
			tagTemplateInput.closest("label").parentElement.querySelector("label").style.display = "none";
			tagTemplateInput.style.display = "none";
			tagTemplateInput.nextElementSibling.style.display = "none"; // 説明文も非表示
		} else {
			containerTemplateSection.style.display = "none";
			// メインのテンプレート入力を表示に戻す
			const labelElement = tagTemplateInput.closest("label");
			if (labelElement) {
				tagTemplateInput.closest("label").parentElement.querySelector("label").style.display = "";
			}
			tagTemplateInput.style.display = "";
			if (tagTemplateInput.nextElementSibling) {
				tagTemplateInput.nextElementSibling.style.display = "";
			}
		}
		...
		switch (selectedType) {
			...
			case "code":
				...
				break;
			case "container":
				placeholder = '【コンテナモード】\n※このモードでは下の「開始部分」と「終了部分」の2つの入力欄を使用します\n※選択した子エリアが、開始部分と終了部分の間に挿入されます';
				break;
			default:
	...
	tagModalForm.addEventListener("submit", (e) => {
		...
		const newContainerTop = containerTopTemplateInput.value;
		const newContainerBottom = containerBottomTemplateInput.value;
		...
		if (newType === 'container') {
			// コンテナモードの場合は、開始部分と終了部分を検証
			if (!newContainerTop || newContainerTop.trim() === '') {
				alert('コンテナ開始部分を入力してください。');
				return;
			}
			if (!newContainerBottom || newContainerBottom.trim() === '') {
				alert('コンテナ終了部分を入力してください。');
				return;
			}
			// 開始部分と終了部分を結合してバリデーション（[CONTENT]を含める）
			const combinedTemplate = newContainerTop + '\n[CONTENT]\n' + newContainerBottom;
			const templateValidation = validateTemplate(combinedTemplate, newType, newAllowScriptTags);
		...
		if (existingId) {
			...
			if (button) {
				...
				button.containerTop = newContainerTop;
				button.containerBottom = newContainerBottom;
			...
			currentPattern.buttons.push({
				...
				containerBottom: newContainerBottom,
	...
	function convertTextToHtmlString(textareaElement) {
		...
		// 静的モードとコンテナモード以外では、入力が空の場合は空文字を返す
		// コンテナモードは子エリアの選択が入力なので、テキストエリアが空でもOK
		if (tagType !== "container" && (!htmlContent || htmlContent.trim() === "")) return "";
		...
		if (tagType === "p-list") {
		...
		} else if (tagType === "container") {
			// コンテナモードの処理 (container)
			// 選択された子エリアのHTMLを取得して結合

			// このコンテナエリアのgroupDivを取得
			const containerGroup = textareaElement.closest(".input-area-group");
			if (!containerGroup) {
				return `<!-- 警告: コンテナエリアが見つかりません -->\n`;
			}

			// 選択された子エリアのIDリストを取得
			const childAreasInput = containerGroup.querySelector(".container-child-areas");
			if (!childAreasInput) {
				return `<!-- 警告: 子エリアの選択情報が見つかりません -->\n`;
			}

			const selectedIds = (childAreasInput.value || "").split(",").filter(id => id.trim());

			// コンテナの開始部分と終了部分を取得
			const containerTop = tagInfo.containerTop || '';
			const containerBottom = tagInfo.containerBottom || '';
			...
			if (selectedIds.length === 0) {
				...
				// 選択された子エリアを変換して結合
				const childHtmlArray = [];

				selectedIds.forEach(areaId => {
					const childArea = document.getElementById(areaId);
					if (childArea) {
						const childTextarea = childArea.querySelector(".input-text-area");
						if (childTextarea) {
							// 子エリアを変換（再帰的に変換されるため、子がコンテナの場合も対応）
							const childHtml = convertTextToHtmlString(childTextarea);
							if (childHtml && childHtml.trim()) {
								// 末尾の改行を削除して追加（最後にまとめて改行を入れるため）
								childHtmlArray.push(childHtml.trim());
							}
						}
					}
				});
				...
				// 子エリアのHTMLを結合
				const contentHtml = childHtmlArray.join("\n");
	...
	// コンテナから要素がドラッグアウトされた際の状態更新関数
	const updateContainerAfterDragOut = (childrenContainer, draggedItemId) => {
		// 親コンテナ要素を取得
		const containerGroup = childrenContainer.closest(".input-area-group");
		if (!containerGroup) return;

		// 隠しフィールドから選択IDリストを取得
		const childAreasInput = containerGroup.querySelector(".container-child-areas");
		if (!childAreasInput) return;

		// IDリストから削除
		const selectedIds = (childAreasInput.value || "").split(",").filter(id => id.trim() && id !== draggedItemId);
		childAreasInput.value = selectedIds.join(",");

		// 対応するチェックボックスを解除
		const checkboxContainer = containerGroup.querySelector(".container-checkbox-list");
		if (checkboxContainer) {
			const checkbox = checkboxContainer.querySelector(`input[type=checkbox][value="${draggedItemId}"]`);
			if (checkbox) {
				checkbox.checked = false;
			}
		}

		// プレースホルダーを表示（子が空になった場合）
		const remainingChildren = childrenContainer.querySelectorAll(".input-area-group");
		const placeholderMsg = childrenContainer.querySelector(".container-placeholder");
		if (remainingChildren.length === 0 && placeholderMsg) {
			placeholderMsg.style.display = "block";
		}
	};

	// テキストエリアを作成し、選択された位置に挿入する関数 (引数を変更)
	const createTextarea = (tagId, tagType, tagName, initialContent = "", customName = "") => {
		areaIndex++;
		...
		// カスタム名前がある場合はそれを使用
		const displayName = customName || tagName;
		groupDiv.setAttribute('aria-label', `入力エリア: ${displayName}`);

		// カスタム名前を保存
		if (customName) {
			groupDiv.setAttribute("data-custom-name", customName);
		}
		...
		tagTextSpan.textContent = displayName;
		tagTextSpan.style.cursor = "text";
		tagTextSpan.title = "ダブルクリックで名前を変更";
		tagLabel.appendChild(tagTextSpan);

		// ダブルクリックで名前変更
		tagTextSpan.addEventListener("dblclick", (e) => {
			e.stopPropagation(); // ドラッグイベントを妨げない
			const currentName = tagTextSpan.textContent;
			const newName = prompt("新しい名前を入力してください:", currentName);

			if (newName !== null && newName.trim() !== "") {
				// タグ名を更新
				tagTextSpan.textContent = newName.trim();

				// カスタム名前をデータ属性として保存
				groupDiv.setAttribute("data-custom-name", newName.trim());

				// aria-labelも更新
				groupDiv.setAttribute('aria-label', `入力エリア: ${newName.trim()}`);
				convertButton.setAttribute('aria-label', `${newName.trim()}入力エリアを個別に変換`);
				deleteButton.setAttribute('aria-label', `${newName.trim()}入力エリアを削除`);

				// プレースホルダーも更新
				const currentPlaceholder = newTextarea.getAttribute("data-placeholder");
				const updatedPlaceholder = currentPlaceholder.replace(/【.*?】/, `【${newName.trim()}】`);
				newTextarea.setAttribute("data-placeholder", updatedPlaceholder);
		...
		groupDiv.addEventListener("drop", (e) => {
			...
			// コンテナから出されたかチェック
			const previousParent = draggingGroup.parentElement;
			const wasInContainer = previousParent && previousParent.classList.contains("container-children-zone");
			...
			// スタイルをリセット
			draggingGroup.style.marginLeft = "";
			draggingGroup.style.opacity = "";

			// コンテナから出た場合、元のコンテナの状態を更新
			if (wasInContainer) {
				updateContainerAfterDragOut(previousParent, draggingGroup.id);
			}
		...
		let placeholderText = `${displayName}タグ用のテキストエリア`;

		if (tagType === "multi") {
			placeholderText = `【${displayName}】マルチラインモード：各行が個別のタグになります`;
		} else if (tagType === "p-list") {
			placeholderText = `【${displayName}】段落+リストモード：通常の行は段落、「-」または「*」で始まる行はリスト項目になります`;
		} else if (tagType === "link-list") {
			placeholderText = `【${displayName}】リンクリストモード：各行に[リンクテキスト](URL)の形式で入力してください`;
		} else if (tagType === "single-link") {
			placeholderText = `【${displayName}】単一リンクモード：[リンクテキスト](URL)の形式で指定。例：クリック[こちら](https://example.com)してください`;
		} else if (tagType === "static") {
			placeholderText = `【${displayName}】静的モード：テンプレートで設定した内容がそのまま出力されます（入力不可）`;
		} else if (tagType === "code") {
			placeholderText = `【${displayName}】コードモード：コードを貼り付けてください。HTML特殊文字（<, >, &など）は自動的にエスケープされます`;
		} else if (tagType === "container") {
			placeholderText = `【${displayName}】コンテナモード：以下のチェックボックスで包含する子エリアを選択してください`;
		} else {
			// 古いモード (single, list, link) のための後方互換性
			placeholderText = `${displayName}エリア。テキストを入力してください。`;
		}
		...
		convertButton.setAttribute('aria-label', `${displayName}入力エリアを個別に変換`);
		...
		deleteButton.setAttribute('aria-label', `${displayName}入力エリアを削除`);
		...
		if (tagType === "container") {
			...
			// コンテナの開始部分を取得して表示
			const tagInfo = getCustomTagInfo(tagId);
			const containerTopHtml = tagInfo?.containerTop || '';
			const containerBottomHtml = tagInfo?.containerBottom || '';

			// コンテナ開始部分の表示エリア
			const containerTopDisplay = document.createElement("div");
			containerTopDisplay.className = "container-visual-top";
			containerTopDisplay.style.backgroundColor = "#e3f2fd";
			containerTopDisplay.style.border = "2px solid #2196F3";
			containerTopDisplay.style.borderBottom = "none";
			containerTopDisplay.style.borderRadius = "4px 4px 0 0";
			containerTopDisplay.style.padding = "8px 12px";
			containerTopDisplay.style.fontSize = "13px";
			containerTopDisplay.style.fontFamily = "monospace";
			containerTopDisplay.style.color = "#1565c0";
			containerTopDisplay.style.whiteSpace = "pre-wrap";
			containerTopDisplay.style.wordWrap = "break-word";
			containerTopDisplay.style.fontWeight = "bold";
			containerTopDisplay.textContent = '▼ ' + (containerTopHtml || '<!-- 開始部分が未設定 -->');

			// 子エリアを入れるコンテナ (視覚的にネストを表現)
			const childrenContainer = document.createElement("div");
			childrenContainer.className = "container-children-zone";
			childrenContainer.style.border = "2px solid #2196F3";
			childrenContainer.style.borderTop = "none";
			childrenContainer.style.borderBottom = "none";
			childrenContainer.style.backgroundColor = "#f5f5f5";
			childrenContainer.style.padding = "10px 10px 10px 30px"; // 左に大きくインデント
			childrenContainer.style.minHeight = "60px";
			childrenContainer.style.position = "relative";

			// ドラッグ&ドロップ: 子エリアをコンテナに入れる
			childrenContainer.addEventListener("dragover", (e) => {
				...
				if (draggingGroup && draggingGroup !== groupDiv) {
					e.dataTransfer.dropEffect = "move";
					childrenContainer.style.backgroundColor = "#c8e6c9"; // ドロップ可能の視覚的フィードバック
					childrenContainer.style.borderColor = "#4CAF50";
				}
			}, { signal });
			...
			childrenContainer.addEventListener("dragleave", (e) => {
				...
				if (e.target === childrenContainer) {
					childrenContainer.style.backgroundColor = "#f5f5f5";
					childrenContainer.style.borderColor = "#2196F3";
				}
			}, { signal });
			...
			childrenContainer.addEventListener("drop", (e) => {
				...
				// コンテナモードのエリアはドロップ不可
				const draggedTagType = draggingGroup.querySelector(".input-text-area")?.getAttribute("data-tag-id");
				const draggedTagInfo = getCustomTagInfo(draggedTagType);
				if (draggedTagInfo && draggedTagInfo.tagType === "container") {
					alert("コンテナの中にコンテナを入れることはできません");
					return;
				}
				...
				// コンテナ内に移動
				childrenContainer.appendChild(draggingGroup);
				draggingGroup.style.marginLeft = "0px";
				draggingGroup.style.opacity = "0.95";

				// 選択状態を更新
				const currentChildren = Array.from(childrenContainer.querySelectorAll(".input-area-group"));
				const selectedIds = currentChildren.map(child => child.id);
				childAreasInput.value = selectedIds.join(",");

				// 対応するチェックボックスをチェック状態にする
				const checkboxContainer = groupDiv.querySelector(".container-checkbox-list");
				if (checkboxContainer) {
					const checkbox = checkboxContainer.querySelector(`input[type=checkbox][value="${draggingGroup.id}"]`);
					if (checkbox) {
						checkbox.checked = true;
					}
				}
			...
			placeholderMsg.className = "container-placeholder";
			...
			placeholderMsg.textContent = "ここに入力エリアをドラッグ&ドロップしてください";
			placeholderMsg.style.pointerEvents = "none"; // ドラッグイベントを妨げない
			childrenContainer.appendChild(placeholderMsg);

			// チェックボックス選択エリア（折りたたみ可能）
			const selectionToggle = document.createElement("button");
			selectionToggle.textContent = "⚙️ 子エリアを選択";
			selectionToggle.style.fontSize = "12px";
			selectionToggle.style.padding = "5px 10px";
			selectionToggle.style.marginBottom = "5px";
			selectionToggle.style.backgroundColor = "#fff";
			selectionToggle.style.border = "1px solid #ccc";
			selectionToggle.style.borderRadius = "3px";
			selectionToggle.style.cursor = "pointer";

			const checkboxContainer = document.createElement("div");
			checkboxContainer.className = "container-checkbox-list";
			checkboxContainer.style.display = "none"; // 初期は非表示
			...
			selectionToggle.addEventListener("click", () => {
				if (checkboxContainer.style.display === "none") {
					...
					selectionToggle.textContent = "⚙️ 選択を閉じる";
				} else {
			...
			// コンテナ終了部分の表示エリア
			const containerBottomDisplay = document.createElement("div");
			containerBottomDisplay.className = "container-visual-bottom";
			containerBottomDisplay.style.backgroundColor = "#e3f2fd";
			containerBottomDisplay.style.border = "2px solid #2196F3";
			containerBottomDisplay.style.borderTop = "none";
			containerBottomDisplay.style.borderRadius = "0 0 4px 4px";
			containerBottomDisplay.style.padding = "8px 12px";
			containerBottomDisplay.style.fontSize = "13px";
			containerBottomDisplay.style.fontFamily = "monospace";
			containerBottomDisplay.style.color = "#1565c0";
			containerBottomDisplay.style.whiteSpace = "pre-wrap";
			containerBottomDisplay.style.wordWrap = "break-word";
			containerBottomDisplay.style.fontWeight = "bold";
			containerBottomDisplay.textContent = '▲ ' + (containerBottomHtml || '<!-- 終了部分が未設定 -->');

			// 子エリア選択情報を保存する隠しフィールド
			const childAreasInput = document.createElement("input");
			childAreasInput.type = "hidden";
			childAreasInput.className = "container-child-areas";
			childAreasInput.value = initialContent || ""; // 保存されたIDリストを復元
			groupDiv.appendChild(childAreasInput);

			// チェックボックスリストを更新する関数
			const updateCheckboxList = () => {
				checkboxContainer.innerHTML = "";
				const allAreas = container.querySelectorAll(".input-area-group");
				let hasOtherAreas = false;
				...
				allAreas.forEach((area) => {
					// 自分自身とコンテナモードのエリアは除外
					if (area === groupDiv) return;
					const areaTagType = area.querySelector(".input-text-area")?.getAttribute("data-tag-id");
					const customTagInfo = getCustomTagInfo(areaTagType);
					if (customTagInfo && customTagInfo.tagType === "container") return;
					...
					// 保存されている選択状態を復元
					// または実際にchildrenContainerに入っているかをチェック
					const isInContainer = childrenContainer.contains(area);
					if (selectedIds.includes(area.id) || isInContainer) {
						checkbox.checked = true;
					}

					// チェック状態が変更されたら、エリアを移動して保存
					checkbox.addEventListener("change", (e) => {
						const childArea = document.getElementById(checkbox.value);
						if (e.target.checked) {
							// チェックされたら子エリアをコンテナ内に移動
							if (childArea && !childrenContainer.contains(childArea)) {
								// プレースホルダーを非表示
								if (placeholderMsg) placeholderMsg.style.display = "none";

								// 元の位置を記憶（データ属性に保存）
								childArea.dataset.originalNextSibling = childArea.nextElementSibling?.id || '';
								childrenContainer.appendChild(childArea);
								// インデント表示
								childArea.style.marginLeft = "0px";
								childArea.style.opacity = "0.95";
							}
							...
							if (childArea && childrenContainer.contains(childArea)) {
								// 元の位置に戻す（コンテナの直後に配置）
								container.insertBefore(childArea, groupDiv.nextElementSibling);
								childArea.style.marginLeft = "";
								childArea.style.opacity = "";

								// 子エリアがなくなったらプレースホルダーを表示
								const remainingChildren = childrenContainer.querySelectorAll(".input-area-group");
								if (remainingChildren.length === 0 && placeholderMsg) {
									placeholderMsg.style.display = "block";
								}
					...
					const numberPrefix = area.querySelector(".input-label-tag .number-prefix")?.textContent || "";
					const tagName = area.querySelector(".input-label-tag span:last-child")?.textContent || "無名エリア";
					const tagLabelText = numberPrefix ? `${numberPrefix} ${tagName}` : tagName;
					const labelText = document.createTextNode(` ${tagLabelText}`);
				...
				if (!hasOtherAreas) {
					checkboxContainer.innerHTML = "<p style='color: #999; margin: 0;'>包含できる子エリアがありません。先に他のタグボタンで入力エリアを作成してください。</p>";
				}
			};
			...
			// 保存されている子エリアを復元（初期ロード時）
			const savedSelectedIds = (childAreasInput.value || "").split(",").filter(id => id.trim());
			setTimeout(() => {
				savedSelectedIds.forEach(areaId => {
					const childArea = document.getElementById(areaId);
					if (childArea && !childrenContainer.contains(childArea)) {
						childrenContainer.appendChild(childArea);
						childArea.style.marginLeft = "0px";
				...
				// 子エリアがあればプレースホルダーを非表示
				if (savedSelectedIds.length > 0 && placeholderMsg) {
					placeholderMsg.style.display = "none";
				}
			}, 100); // 少し遅延させてDOM構築後に実行

			// 他のエリアが追加・削除されたときにリストを更新
			// MutationObserverでコンテナの変更を監視
			...
			// クリーンアップ時にオブザーバーも停止
			const originalCleanup = groupDiv._cleanup;
			groupDiv._cleanup = () => {
				observer.disconnect();
				if (originalCleanup) originalCleanup();
			};

			// 構造を組み立て: 開始タグ → 子エリア → 終了タグ
			groupDiv.appendChild(containerTopDisplay);

			// 選択ボタンとチェックボックスを子エリアコンテナに配置
			childrenContainer.appendChild(selectionToggle);
			childrenContainer.appendChild(checkboxContainer);

			groupDiv.appendChild(childrenContainer);
			groupDiv.appendChild(containerBottomDisplay);
		}

		// ボタンコンテナを作成（縦並び）
		const buttonContainer = document.createElement("div");
		buttonContainer.className = "button-container-vertical";
		...
		convertAllButton.addEventListener("click", function () {
			...
			if (allTextareas.length === 0) {
				...
				allTextareas.forEach((textarea) => {
					// コンテナ内にある子エリアはスキップ（コンテナ自体の変換時に含まれるため）
					const isInsideContainer = textarea.closest(".container-children-zone") !== null;
